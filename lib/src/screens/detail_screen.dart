...
class _SkinDetailScreenState extends State<SkinDetailScreen> {
	...
	Color _rarityColor(String rarity) {
		...
		if (r.contains('mil') || r.contains('mil-spec')) return const Color(0xFF2979FF); // Blue
	...
	Widget build(BuildContext context) {
		...
		final fullImage = (skin.image.startsWith('assets/') || skin.image.startsWith('images/'))
			? skin.image
			: 'assets/images/full/${skin.image}';
		final thumbImage = skin.thumbnail.startsWith('assets/') ? skin.thumbnail : 'assets/images/thumbs/${skin.thumbnail}';
																				...
																				return AnimatedBuilder(
																					...
																					builder: (context, child) {
																						...
																						return Transform(
																							...
																							transform: Matrix4.identity()..scale(scale, scale)..rotateZ(rot),
																							child: child,
																						);
																					},
																					child: Image.asset(
										...
										children: [
											Expanded(
												child: Column(
													crossAxisAlignment: CrossAxisAlignment.start,
													children: [
														Text(skin.name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
														const SizedBox(height: 4),
														Text(skin.weapon, style: const TextStyle(color: Colors.grey, fontSize: 13)),
													],
												),
											),
											Container(
												...
												child: Text(skin.rarity, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 13)),
											),
										],
									),
																			...
																			children: [
																				Row(
																					children: [
																						...
																						const Spacer(),
																						Text('\$${skin.price.toStringAsFixed(2)}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
																					],
																				),
